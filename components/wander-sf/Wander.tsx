...
import { css } from "@/lib/wander-sf/css";
import { Img } from "./Img";
import * as geo from "@/lib/wander-sf/geo";
import { MODEL, SENS, OUT, BK, wowImgId, type Day, type Segment } from "@/lib/wander-sf/trip";
import {
  defaultState,
  loadPersisted,
  persist,
  type WanderState,
} from "@/lib/wander-sf/state";
    ...
    return (
      <div className="scroll" style={css("height:100%;overflow-y:auto;padding:0 0 104px")}>
        ...
        <div style={css("position:relative;height:376px")}>
          ...
          <div style={css("position:absolute;left:22px;right:22px;bottom:22px;color:#fff")}>
            <div style={css("display:flex;align-items:center;gap:7px;font-size:13px;font-weight:500;color:rgba(255,255,255,.9)")}>
              <I.PinFill size={14} /> California, USA
            </div>
            <div style={css("font-family:'Newsreader',serif;font-size:60px;font-weight:500;letter-spacing:-1px;line-height:.94;margin-top:8px")}>San Francisco</div>
            <div style={css("font-size:14px;font-weight:500;color:rgba(255,255,255,.92);margin-top:12px")}>Jul 31 – Aug 4, 2026{awayLabel}</div>
          </div>
        </div>
        ...
        <div style={css("margin:-30px 18px 0;position:relative;background:#fff;border-radius:18px;padding:6px;display:flex;box-shadow:0 20px 40px -26px rgba(24,27,51,.6)")}>
          {[
            { value: "5", label: "Days", divider: true },
